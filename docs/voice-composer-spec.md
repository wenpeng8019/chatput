...
本文件描述会话页底部「语音操作面板」（voice composer）的视觉结构、手势交互、阈值参数与触感反馈规格，作为 iOS / Android 双端对齐的依据。第 §9 节另述会话页的「远程屏幕下拉幕布」（视频窗口）交互与标准。

- iOS 实现：`mobile-iphone/Sources/App/ChatView.swift`（语音面板）。**视频幕布 iOS 端尚未实现**（见 §9）。
- Android 实现：
  - 语音面板：`mobile-android/app/src/main/java/com/chatput/app/ChatActivity.kt`、`DirectionHintsView.kt`、`res/layout/activity_chat.xml`
  - 视频幕布：`ScreenCurtainController.kt`（手势 / 平移 / 开合）、`ScreenPanelController.kt`（采集 / 渲染 / 视口）、`activity_chat.xml`（`screen_panel` 等）
