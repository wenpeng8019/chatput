...
本文件描述会话交互页面的视觉结构、手势交互、阈值参数与触感反馈规格，作为 iOS / Android 双端对齐的依据。涵盖语音输入、文字输入、光标控制、远程屏幕幕布等全部交互层。第 §9 节另述「远程屏幕下拉幕布」（视频窗口）交互与标准。

- iOS 实现：`mobile-iphone/Sources/App/ChatView.swift`（语音面板）。**视频幕布 iOS 端尚未实现**（见 §9）。
- Android 实现：
  - 语音面板：`ChatActivity.kt`（§Voice Composer / Composer Buttons）、`DirectionHintsView.kt`、`activity_chat.xml`
  - 文字输入：`ChatActivity.kt`（§Text Input Panel）
  - 视频幕布：`ScreenCurtainController.kt`（手势 / 动画 / keyboardLift / collapseZone）、`ScreenPanelController.kt`（渲染 / 视口 / 触控转鼠标）、`MinimapView.kt`（缩略图）、`activity_chat.xml`（`screen_panel` 等）
  - Input-lost 替代交互：`ChatActivity.kt`（§Input State）、`ic_dpad.xml`、`ic_nav_esc.xml`
  - Header 菜单：`popup_header_actions.xml`
  - 摇晃检测：`ShakeDetector.kt`
...
会话页可在语音交互之上叠加远程桌面画面。它不是独立页面，而是一块从屏幕**顶部下拉、像幕布一样覆盖标题与消息列表**的悬浮视频面板，与语音面板共存于同一个会话页。

- Android 实现：`ScreenCurtainController`（幕布开合 / 手势 / 键盘平移）+ `ScreenPanelController`（采集、渲染、视口拖动、小地图）。
- **iOS 现状**：iOS 会话页（`ChatView.swift`）目前**只有语音面板，尚无幕布 / 远程屏幕功能**。本节为 Android 既有实现的规格；iOS 后续补齐时以此为对齐基准。

### 9.1 形态与生命周期

```
   ┌──────────── 视频面板（screen_panel） ────────────┐  ← 顶部延展到状态栏下（沉浸式）
   │  ▓▓▓▓▓▓▓▓  远程桌面画面（SurfaceViewRenderer） ▓▓│
   │  ▒ 小地图(minimap)   状态(screen_status)        │
   │           ⎯⎯ 握把(screen_grab) ⎯⎯               │  ← 顶部可抓握收起
   └──────────────────────────────────────────────────┘  ← 底边投影(screen_shadow)
        ↑ 下方即语音面板 hint / composer（幕布覆盖其上）
```

- **打开入口**：标题栏「更多」菜单 →「查看屏幕」（`action_view_screen` → `openScreenView()`）。未连接桌面端时 toast「未连接到桌面端」，不展开。
- **打开 = 开始采集**：幕布下拉到位时 `ScreenPanelController.start()` 启动 WebRTC 采集；**完全收起 = 停止采集**（`stop()`），不在后台空耗。
- **平移模型**：面板用 `translationY` 在 `[-panelHeight, 0]` 间滑动——`0` = 完全打开，`-panelHeight` = 完全收起。
- **收起即 GONE**：收起后面板置 `View.GONE`（而非 `INVISIBLE`）。因为 `SurfaceViewRenderer` 的 surface 是独立合成层，`INVISIBLE`/`translationY` 约束不住，会在屏幕顶部泄露出透明 / 黑边；`GONE` 把面板移出布局彻底消除泄露。
- **首帧前盖板**：采集启动到首帧到达前，`screen_cover`（黑）盖住透明的 SurfaceView「打洞」，避免穿透闪烁；首帧（`onVideoTrack`）后隐藏。

### 9.2 开合手势

| 手势 | 触发区 | 行为 |
|---|---|---|
| 下拉打开 | 标题区（`chatHeader` 作 `headerDragHost`） | 仅在**未打开**时接管；下滑 `> touchSlop` 开始跟手，面板随手指下移 |
| 顶部握把收起 / 继续拉 | `screen_grab`（顶部 64dp） | 抓握上滑收起、下滑继续展开，带 `VelocityTracker` |
| 底部热区收起 | `screen_collapse_zone`（hint band + 向上溢出 10dp） | 仅在**已打开**时接管；上滑 `< -touchSlop` 拖动收起整块面板 |

- **松手吸附（settle）**：按速度或位置二选一判定开 / 合——
  - 速度 `vy > 800` → 开；`vy < -800` → 合；
  - 否则按进度 `progress > 0.4` → 开，反之合。
- **开合动画**：`animateTo`，`280ms`，`DecelerateInterpolator(1.4f)`；投影 `screen_shadow` 用相同 `translationY` 跟随贴在底边下。

> **手势要点**：`headerDragHost` 的 `ACTION_DOWN` 必须返回 `!opened`（未打开时消费），否则后续 `MOVE/UP` 不投递、整个下拉手势失效。

### 9.3 键盘联动 — 窗帘式上推（本轮重点）

呼出文字输入栏（§4）并弹出系统键盘时，幕布需让出输入框 + 键盘的空间。**标准是「画面尺寸不变、整块向上平移」**（窗帘式），而非压缩面板高度。

- **正确做法（平移）**：面板布局约束**始终不变**（底边钉 `hint`），只在键盘弹起时给一个负的 `translationY = -keyboardLift`，把整块面板上推；画面比例与大小保持不变。
  - `ScreenCurtainController.setKeyboardLift(lift)`：仅当 `opened` 且无手势进行、无开合动画运行（`animator?.isRunning != true`）时生效。
- **逐帧驱动（与键盘同步）**：在 `WindowInsetsAnimationCompat.Callback.onProgress` 里逐帧计算并下发 `lift`，使面板与键盘**同一条动画曲线、同帧移动**：
  - `lift = (screen_panel.bottom − inputTop).coerceAtLeast(0)`
  - `inputTop = root.height − imeBottom − inputH`（`inputH` 为输入栏高度，缓存于 `lastInputHeightPx`）。
  - 同帧设 `textInputCard.bottomMargin = imeBottom`，使输入栏贴键盘上沿。
- **收起平滑**：`lift` 是 `imeBottom` 的线性函数，键盘回落时随之平滑收敛到 `0`，面板回位。

> **两个反面教训（勿重蹈）**：
> 1. **不要改 `bottomToTop` 约束去压面板高度**——`SurfaceViewRenderer` 为 `SCALE_ASPECT_FIT`，面板变矮会触发画面重缩放（视觉上「缩了一圈」），这不是用户要的。
> 2. **不要用 `textInputCard.top` 算 `lift`**——输入栏隐藏时变 `GONE`，其 `top` 在 ConstraintLayout 中折叠跳变，会导致收键盘时面板「先猛弹一下再回落」。用 `root.height − imeBottom − inputH` 推算可避免。
> 3. `setKeyboardLift` 的守卫别写 `animator == null`——`open()` 动画结束后 `animator` 不为 null，会让平移永远被跳过（面板纹丝不动）。用 `animator?.isRunning != true`。

### 9.4 远程画面操作（ScreenPanelController）

- **视口（viewport）**：请求的子区域 = 手机渲染区物理像素 ÷ 手机自身 `density`（retina≈2），渲染器 `SCALE_ASPECT_FIT` 上采样填满 → 清晰 1:1。
- **主画面拖动**：在画面上拖动平移视口；两轴共用同一显示缩放 `dispScale = min(rendererW/vpW, rendererH/vpH)`（含 retina），避免按轴各算在黑边轴出错。
- **视口权威**：`viewportInited` 后手机为视口唯一权威，**不再**用桌面回填的 `applied` 覆盖本地 `vpX/vpY`（否则与拖动乐观更新打架产生颤动）。
- **小地图（minimap）**：窗口尺寸以 `screen-meta` 的 `win` 为权威（不取缩略图 JPEG 像素）；可在小地图上拖动视口框。

### 9.5 关键参数（幕布）

| 参数 | 值 | 含义 |
|---|---|---|
| 开合动画时长 | `280ms` | `DecelerateInterpolator(1.4f)` |
| settle 速度阈值 | `±800` px/s | 超过即按方向吸附开 / 合 |
| settle 位置阈值 | `progress > 0.4` | 速度不足时按展开比例判定 |
| 握把高度 | `64dp` | `screen_grab` 抓握区 |
| 收起热区溢出 | `10dp` | `screen_collapse_zone` 向上溢出，覆盖 hint band |
| `keyboardLift` | 动态 | `panel.bottom − inputTop`，逐帧随 `imeBottom` |

### 9.6 待优化

- **首连卡顿 / 模糊**：WebRTC 编码器 / 码率 ramp-up，过几秒好转。方向：请求 keyframe、提高初始码率下限、调 `degradationPreference`。
- **SurfaceView 跟手性**：SurfaceView 是独立合成层，`translationY` 平移时层比 View 层慢 1~2 帧。如需更跟手可改用 `org.webrtc.TextureViewRenderer`（getstream WebRTC 库自带），随 View 层同帧合成。
- **~首连卡顿/模糊~**：✅ 已通过固定码率选项（`disableDynamicBitrate`）+ 初始码率下限 4Mbps + degradationPreference=DISABLED 解决。
- **~采集失败反馈~**：✅ `screen-error` 协议，Android 端关闭幕布并显示错误。

---
## 10. 文字输入面板

- Android 实现：`ChatActivity.kt` §Text Input Panel（`bindTextInput`, `bindTextInputSwipeDown`, `showTextInput`, `hideTextInput`）
...
| 手势 | 触发区 | 行为 |
|---|---|---|
| 上拉呼出 | `drag_handle`（全宽感应层） | 向上拖 > `pullMax × 0.55` → 展开文字输入，语音框上移渐出 |
| 轻点呼出 | `drag_handle`，落在 `composer_card` 内 | 同上 |
| 下滑收起 | `EditText` 上 | 向下拖 > 28dp → 切回语音面板 |
| 上滑关幕布 | `EditText` 上 | 向上拖 > 28dp + 幕布打开时 → `screenCurtain.close()` |

- 方向优先判定：MOVE 阶段确认方向后才拦截，否则透传给 EditText；方向确认瞬间发 `ACTION_CANCEL` 清除按压态防误触。

### 10.3 键盘联动
- 键盘收起时 `WindowInsets` 检测 → 自动 `hideTextInput()`

---
## 11. Input 丢失替代交互（D-pad 模式）

桌面端输入控件暂时不可用时，移动端切换为 D-pad 模式。

- 协议：`session-input-lost` / `session`（恢复）
- Android：`ChatActivity.kt` §Input State

### 11.1 控件切换

| 按钮 | 正常 | D-pad |
|---|---|---|
| 中间 | `ic_mic` + 按住说话+拖动 | `ic_dpad` + 点击四象限 → `cursorUp/Down/Left/Right` |
| 左侧 | `ic_nav_backspace` + 点击退格/长按清空 | `ic_nav_esc` + 点击 `escape` |
| 右侧 | `ic_nav_enter` + 点击 shiftEnter/长按 enter | 点击直接 `enter` |
| 输入框 | 可用 | 禁用 |
| Hint | "按住说话" | "↑↓←→ 移动光标" |
...
- Android：`popup_header_actions.xml` + `ChatActivity.showHeaderMenu()`

| 选项 | 动作 | 备注 |
|---|---|---|
| 查看屏幕 | `screenCurtain.open()` | |
| 撤销 | `undo` (Cmd+Z) | 也可摇晃手机触发 |
| 全选 | `selectAll` (Cmd+A) | |
| 清空 | `clear` (全选+退格) | toast "已清空" |

---
## 13. 幕布收起热区（collapse_zone）

- Android：`ScreenCurtainController.bindCollapseZone()` + `screen_collapse_zone`

### 13.1 位置与大小

| 模式 | 高度 | 位置 |
|---|---|---|
| 语音模式（键盘未弹） | 72dp | 幕布底边内侧 10dp + hint 区全高 |
| 文本模式（键盘弹出） | 20dp | 幕布底边内外各 10dp |

- 约束：`bottom_toBottomOf_screen_panel` + `marginBottom=-10dp`
- 可见性：`open()` 时 VISIBLE，`close()` 时 GONE
- 键盘联动：`translationY` 跟随 panel（`syncShadow`）

### 13.2 手势
- 上划 > 50dp → 直接 `close()`；不足 → `settle()` 回弹
- MOVE 始终 return true（否则未超 threshold 返回 false 会终止手势流）

---
## 14. Android 代码架构

`ChatActivity.kt` MARK 分区（~620 行）：
...
├── Header & Messages    — menu / resend / delete / balloon
